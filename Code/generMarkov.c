...
void  usage()
{
	printf("usage : GenerMatrix -f filename UP DOWN\n");
...
char *argv[];
{
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			UP = atoi(*(++argv));
			DOWN =atoi (*(++argv));
			//printf("Gen Markov : UP = %d et DOWN = %d\n",UP,DOWN);
			pf1=fopen(s1,"w");
			pf2=fopen(s2,"w"); 
			break;
		}
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R ,UP, DOWN); //equa
#if (DEBUG==1)
			PrintEtat(F);
#endif
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R, UP, DOWN);
#if (DEBUG==1)
			PrintEtat(F);
#endif
